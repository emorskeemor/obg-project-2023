...
class CliManager:
    ...
    Use @non_command to ignore a function as a command.
    '''
    ...
    def register(self, func, instance=False, name:str=None):
        ...
        if getattr(self, func.__name__, None) is not None:
            raise CommandAlreadyExists(
                "'%s' is already registered as command or a method that already exists" % func.__name__)
        setattr(self, func.__name__, func)

    @non_command
    def str_to_boolean(self, string:str):
        if string == "t":
            return True
        return False
    ...
    @non_command
    def str_to_iter(self, string:str, delimeter=None):
